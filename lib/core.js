...
function profile(rows, fields, opts) {
  ...
  var compositeSpecs = (opts.compositeKeys || []).map(function (colNames) {
    return { columns: colNames, seen: Object.create(null), repeats: 0 };
  });
  ...
  for (var i = 0; i < n; i++) {
    ...
    compositeSpecs.forEach(function (spec) {
      var joined = spec.columns.map(function (col) { return row[col]; }).join('\u241F');
      if (spec.seen[joined]) spec.repeats++; else spec.seen[joined] = 1;
    });
  ...
  cols.forEach(function (c) {
    ...
    if (c.nums.length) {
      ...
      c.mean = c.nums.reduce(function (a, v) { return a + v; }, 0) / c.nums.length;
      var variance = c.nums.reduce(function (a, v) { return a + (v - c.mean) * (v - c.mean); }, 0) / c.nums.length;
  ...
  var p = {
    rows: n, cols: cols, dup: dup, empty: n === 0,
    compositeKeys: compositeSpecs.map(function (spec) {
      return { columns: spec.columns, unique: spec.repeats === 0, unknown: false, repeats: spec.repeats };
    })
  };
  p.pii = scanPII(cols);
  return p;
...
function issues(p) {
  ...
  if (p.empty || p.rows === 0) {
    add('critical', 60, [1, 2], 'File has no data rows',
      'The file parsed successfully but contains zero rows below the header. ' +
      'Every downstream check depends on data being present — confirm the export actually ran.');
    return o;
  }
...
var COLUMN_RULES = ['required', 'unique', 'max_null_pct', 'type', 'pii', 'description',
                    'allowed_values', 'min', 'max', 'max_mean_deviation_pct'];
var GLOBAL_RULES = ['allow_new_columns', 'allow_missing_columns', 'max_row_drop_pct',
                    'max_duplicate_pct', 'null_tolerance_pp', 'min_rows', 'unique_together',
                    'allow_empty', 'value_deviation_pct', 'row_deviation_pct',
                    'conditional', 'sla', 'no_regression', 'gates'];
...
function buildContract(p, opts) {
  ...
  var columns = p.cols.map(function (c) {
    ...
    // baseline numeric stats — inferred, not declarable, since a human can't
    // meaningfully hand-pick a mean. Only recorded for columns with numeric
    // content, and only used if a deviation threshold is set somewhere.
    if (c.numCount || c.mean !== undefined) {
      spec.stats = {
        mean: round(c.mean), stddev: round(c.stddev || 0),
        sum: round((c.mean || 0) * (c.numCount || c.nums.length || 0))
      };
    }
...
function checkContract(contract, p) {
  ...
  if (p.empty || p.rows === 0) {
    fail(rules.allow_empty ? 'info' : 'critical', 'empty_file',
      rules.allow_empty
        ? 'File has zero data rows. Allowed by contract, but confirm this run was expected to be empty.'
        : 'File has zero data rows. Every other check is skipped since there is nothing to check.');
    return v;
  }
  ...
  contract.columns.forEach(function (spec) {
    ...
    // Payload / value deviation: the column can be perfectly typed, fully
    // populated, and still be wrong — a currency conversion bug or a unit
    // change shifts every value by the same factor and passes every check
    // above. Comparing the new mean against the baseline catches that.
    var devThreshold = pick(spec.max_mean_deviation_pct, rules.value_deviation_pct);
    if (devThreshold != null && spec.stats && got.mean != null) {
      var baseMean = spec.stats.mean;
      var denom = Math.abs(baseMean) > 1e-9 ? Math.abs(baseMean) : 1;
      var deviationPct = Math.abs(got.mean - baseMean) / denom * 100;
      if (deviationPct > devThreshold)
        fail(deviationPct > devThreshold * 3 ? 'critical' : 'warning', 'value_deviation',
          'Column "' + spec.name + '" average moved from ' + baseMean + ' to ' + round(got.mean) +
          ' (' + deviationPct.toFixed(0) + '% change), beyond the ' + devThreshold + '% allowance.',
          spec, 'max_mean_deviation_pct');
    }
  });
  ...
  if (rules.unique_together && rules.unique_together.length) {
    var got = {};
    (p.compositeKeys || []).forEach(function (ck) { got[ck.columns.join('\u241F')] = ck; });
    rules.unique_together.forEach(function (colGroup) {
      var key = colGroup.join('\u241F'), gk = got[key];
      var label = '(' + colGroup.join(', ') + ')';
      if (!gk || gk.unknown) {
        v.push({ severity: 'warning', code: 'composite_uniqueness_unknown', origin: 'declared',
          columns: colGroup,
          message: 'Could not verify uniqueness of ' + label + ' — the columns may be missing from this file.' });
      } else if (!gk.unique) {
        v.push({ severity: 'critical', code: 'composite_uniqueness_lost', origin: 'declared',
          columns: colGroup,
          message: 'The combination ' + label + ' is no longer unique — ' + gk.repeats +
                   ' repeated combination(s) found.' });
      }
    });
  }

  // Conditional rules — business logic checks
  if (rules.conditional && rules.conditional.length && p.conditionalResults) {
    p.conditionalResults.forEach(function (cr) {
      if (!cr.passed) {
        v.push({
          severity: cr.violated > cr.matched * 0.5 ? 'critical' : 'warning',
          code: 'conditional_rule_violated',
          origin: 'declared',
          column: null,
          message: 'Rule "' + cr.label + '" violated in ' + cr.violated + ' of ' +
            cr.matched + ' applicable rows.'
        });
      }
    });
  }
  ...
  // Apply gates: override the severity of any check code.
  // "gates": { "sla_breach": "info", "row_count_deviation": "warning", "empty_file": "off" }
  // "off" removes the violation entirely. Everything else remaps the severity.
  var gates = rules.gates || {};
  if (Object.keys(gates).length) {
    v = v.filter(function (x) {
      var gate = gates[x.code];
      if (gate === undefined) return true;  // no override, keep as-is
      if (gate === 'off' || gate === 'ignore' || gate === false) return false;  // suppress
      if (gate === 'critical' || gate === 'warning' || gate === 'info') {
        x.severity = gate;
        x.gated = true;  // mark so output can show it was overridden
        return true;
      }
      return true;  // unrecognized gate value, keep original
    });
  }
