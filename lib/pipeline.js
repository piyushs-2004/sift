...
function Accumulator(fields, opts) {
  ...
  this.cols = fields.map(function (name) {
    return {
      ...
      numCount: 0, numSum: 0, numSumSq: 0, numMin: Infinity, numMax: -Infinity,
  ...
  // Composite (multi-column) uniqueness, e.g. unique_together: [["order_id","region"]].
  // A single column's key tracking can't answer "is this pair unique" — each
  // declared combination gets its own hash bag over the joined values of just
  // those columns. Composite keys are never inferred, only declared, so there
  // is no default list here.
  this.compositeSpecs = (opts.compositeKeys || []).map(function (cols) {
    return { columns: cols, bag: new HashBag(), candidate: true, unknown: false };
  });

  this.conditionalTracker = new ConditionalTracker(opts.conditionalRules || []);
}
...
Accumulator.prototype.push = function (row, index) {
  ...
  for (var ci = 0; ci < this.cols.length; ci++) {
    ...
    if (k === 'integer' || k === 'decimal') {
      var f = parseFloat(t);
      if (isFinite(f)) {
        c.numCount++; c.numSum += f; c.numSumSq += f * f;
  ...
  for (var si = 0; si < this.compositeSpecs.length; si++) {
    var spec = this.compositeSpecs[si];
    if (!spec.bag) continue;
    var joined = '';
    for (var ki = 0; ki < spec.columns.length; ki++) joined += row[spec.columns[ki]] + '\u241F';
    if (!spec.bag.add(hash53(joined))) { spec.unknown = true; spec.bag = null; }
  }

  this.conditionalTracker.push(row);
...
Accumulator.prototype.finalize = function () {
  ...
  this.cols.forEach(function (c) {
    ...
    if (c.numCount) {
      ...
      var variance = c.numSumSq / c.numCount - c.mean * c.mean;
      c.stddev = variance > 0 ? Math.sqrt(variance) : 0;
  ...
  var compositeKeys = this.compositeSpecs.map(function (spec) {
    var out = { columns: spec.columns, unique: false, unknown: !!spec.unknown, repeats: 0 };
    if (spec.bag) {
      var d = spec.bag.distinct();
      if (d >= 0) { out.unique = (d === n); out.repeats = n - d; }
  ...
  var p = { rows: n, cols: this.cols, dup: this.dup, dup_approximate: this.dupTruncated,
            empty: n === 0, compositeKeys: compositeKeys,
            conditionalResults: this.conditionalTracker.results() };
...
function rowValidator(contract) {
  ...
  var allowedSets = {};
  var condRules = (contract.rules && contract.rules.conditional) || [];
  ...
  return function (row) {
    ...
    var condErrs = conditionalRowErrors(row, condRules);
    if (condErrs) { errs = (errs || []).concat(condErrs); }
    return errs;
  };
}
 ...
 * Row-count history.
 *
 * A contract's baseline is a single snapshot. Real files vary day to
 * day — 9,800 one day, 10,200 the next. The history file is a tiny
 * JSON log that records each run's row count, so the check can compare
 * against the rolling average instead of a single frozen number.
 *
 * File shape:
 *   { runs: [ { date, rows, file, passed }, ... ] }
 *
 * Design: the history is append-only and capped by count. Older
 * entries roll off. The file is small enough to read/write whole.
 * ------------------------------------------------------------------ */
var fs = require('fs');
...
function loadHistory(histPath) {
  try {
    var raw = fs.readFileSync(histPath, 'utf8');
...
function appendHistory(histPath, entry, maxRuns) {
  maxRuns = maxRuns || 90;
  var h = loadHistory(histPath);
  h.runs.push(entry);
  if (h.runs.length > maxRuns) h.runs = h.runs.slice(-maxRuns);
  var dir = path.dirname(histPath);
  if (dir && dir !== '.' && !fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(histPath, JSON.stringify(h, null, 2));
  return h;
}

function rowCountStats(history, excludeLast) {
  var runs = history.runs || [];
  if (excludeLast && runs.length > 0) runs = runs.slice(0, -1);
  if (!runs.length) return null;
  var counts = runs.map(function (r) { return r.rows; });
  var n = counts.length;
  var sum = counts.reduce(function (a, v) { return a + v; }, 0);
  var mean = sum / n;
  var variance = counts.reduce(function (a, v) { return a + (v - mean) * (v - mean); }, 0) / n;
...
function checkRowDeviation(currentRows, history, tolerancePct) {
  var stats = rowCountStats(history, false);
  if (!stats || stats.count < 3) return null;  // not enough history to judge
  var mean = stats.mean;
  if (mean === 0) return null;
  var deviationPct = Math.abs(currentRows - mean) / mean * 100;
  if (deviationPct <= tolerancePct) return null;
  var direction = currentRows < mean ? 'below' : 'above';
  return {
    severity: deviationPct > tolerancePct * 3 ? 'critical' : 'warning',
    code: 'row_count_deviation',
    origin: 'declared',
    column: null,
    message: 'File has ' + currentRows.toLocaleString() + ' rows, ' +
      deviationPct.toFixed(0) + '% ' + direction + ' the rolling average of ' +
      mean.toLocaleString() + ' (over ' + stats.count + ' recent runs). ' +
      'Tolerance is ' + tolerancePct + '%.',
    stats: stats
  };
}

/* ------------------------------------------------------------------ *
 * SLA — did the file arrive on time?
 *
 * sla spec:  { by: "06:00", timezone: "+05:30" }  (optional tz)
 * Compares the file's mtime against the deadline for that calendar day.
 * ------------------------------------------------------------------ */
function checkSLA(mtime, slaSpec) {
  if (!mtime || !slaSpec || !slaSpec.by) return null;
  var parts = slaSpec.by.match(/^(\d{1,2}):(\d{2})$/);
  if (!parts) return null;

  var deadlineH = +parts[1], deadlineM = +parts[2];
  var t = new Date(mtime);
  var fileH = t.getUTCHours(), fileM = t.getUTCMinutes();

  // apply timezone offset if given, e.g. "+05:30" or "-04:00"
  if (slaSpec.timezone) {
    var tzm = slaSpec.timezone.match(/^([+-])(\d{2}):?(\d{2})$/);
    if (tzm) {
      var off = (+tzm[2] * 60 + +tzm[3]) * (tzm[1] === '-' ? -1 : 1);
      var totalMin = fileH * 60 + fileM + off;
      fileH = Math.floor(((totalMin % 1440) + 1440) % 1440 / 60);
      fileM = ((totalMin % 1440) + 1440) % 1440 % 60;
    }
  }

  var fileMin = fileH * 60 + fileM;
  var deadlineMin = deadlineH * 60 + deadlineM;
  var lateMin = fileMin - deadlineMin;

  if (lateMin <= 0) return null; // on time

  return {
    severity: lateMin > 120 ? 'critical' : 'warning',
    code: 'sla_breach',
    origin: 'declared',
    column: null,
    message: 'File arrived at ' + pad2(fileH) + ':' + pad2(fileM) +
      ', ' + fmtLate(lateMin) + ' past the ' + slaSpec.by + ' SLA deadline.'
  };
}
 ...
 * No-regression gate
 *
 * The current file's quality score must be >= the last passing run's
 * score. Prevents gradual erosion where each file is slightly worse
 * but never bad enough to trigger any single rule.
 * ------------------------------------------------------------------ */
function checkRegression(currentScore, history) {
  var runs = (history && history.runs) || [];
  // find the most recent run that passed and has a recorded score
  var lastPassing = null;
  for (var i = runs.length - 1; i >= 0; i--) {
    if (runs[i].passed && runs[i].score != null) { lastPassing = runs[i]; break; }
  }
  if (!lastPassing) return null; // no baseline to compare against

  if (currentScore >= lastPassing.score) return null;

  var drop = lastPassing.score - currentScore;
  return {
    severity: drop > 20 ? 'critical' : 'warning',
    code: 'quality_regression',
    origin: 'declared',
    column: null,
    message: 'Quality score dropped from ' + lastPassing.score + ' to ' + currentScore +
      ' (-' + drop + ' points) compared to the last passing run (' + lastPassing.date + ').'
  };
}

/* ------------------------------------------------------------------ *
 * Conditional rules — business logic as contract checks.
 *
 ...
 *   ]
 *
 * Supported when-conditions: equals, in, not_null, is_null
 * Supported then-conditions: not_null, equals, in, not_equals, min, max
 * ------------------------------------------------------------------ */
function evalCondition(row, cond) {
  var v = row[cond.column];
  var blank = core.isNull(v);
  var t = blank ? '' : String(v).trim();

  if (cond.is_null) return blank;
  if (cond.not_null) return !blank;
  if (blank) return false; // can't match equals/in on a null

  if (cond.equals !== undefined) return t === String(cond.equals);
  if (cond.not_equals !== undefined) return t !== String(cond.not_equals);
  if (cond.in) return cond.in.some(function (x) { return t === String(x); });
  if (cond.min != null) { var n = parseFloat(t); return isFinite(n) && n >= cond.min; }
  if (cond.max != null) { var n2 = parseFloat(t); return isFinite(n2) && n2 <= cond.max; }
  if (cond.matches) { try { return new RegExp(cond.matches).test(t); } catch (e) { return false; } }
  return true;
}

/* ConditionalTracker: counts violations per rule during streaming. */
function ConditionalTracker(rules) {
  this.rules = (rules || []).map(function (r, i) {
    return {
      id: i, when: r.when, then: r.then,
      label: r.label || (r.when.column + '=' + (r.when.equals || r.when.in || '?') +
        ' → ' + r.then.column + ' ' + (r.then.not_null ? 'not null' : r.then.equals || '?')),
      matched: 0, violated: 0
    };
  });
}

ConditionalTracker.prototype.push = function (row) {
  for (var i = 0; i < this.rules.length; i++) {
    var r = this.rules[i];
    if (evalCondition(row, r.when)) {
      r.matched++;
      if (!evalCondition(row, r.then)) r.violated++;
    }
  }
};

ConditionalTracker.prototype.results = function () {
  return this.rules.map(function (r) {
    return {
      label: r.label, matched: r.matched, violated: r.violated,
      passed: r.violated === 0
    };
  });
};

/* Add conditional checking to the row validator for quarantine */
function conditionalRowErrors(row, conditionalRules) {
  if (!conditionalRules || !conditionalRules.length) return null;
  var errs = null;
  for (var i = 0; i < conditionalRules.length; i++) {
    var r = conditionalRules[i];
    if (evalCondition(row, r.when) && !evalCondition(row, r.then)) {
      errs = errs || [];
      errs.push('rule: when ' + r.when.column + ' then ' + r.then.column +
        (r.then.not_null ? ' must not be null' : ' must be ' + (r.then.equals || r.then.in || '?')));
    }
  }
  return errs;
}
...
module.exports = {
  ...
  loadHistory: loadHistory, appendHistory: appendHistory,
  rowCountStats: rowCountStats, checkRowDeviation: checkRowDeviation,
  checkSLA: checkSLA, checkRegression: checkRegression,
  ConditionalTracker: ConditionalTracker, conditionalRowErrors: conditionalRowErrors,
